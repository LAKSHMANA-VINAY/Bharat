...
if(isset($_POST['submit']))
{
    ...
    if($count>0){
        ...
        $count2= $row1['u_count'];
        if($count2>0){
            header("Location:member_register.php?error=Account already exists");
            exit();
        }
        else
        {
            ...
            header("Location:member_register.php?success=You registred successfully");
            exit();
        }
    }
    else
    {
        header("Location:member_register.php?error=Sry Group Id Not exists");
        exit();
    }
}
?>